    ...
    def from_npz(cls, path) -> "Differential":
        with np.load(path, allow_pickle=False) as x:
            return cls(
                ...
                x["mu_x"], x["sig2_x"], x["loglik_mu"],
                x["loglik_mu_sig2"], x["log_sig2_prior"],
            )
...
class DifferentialModel:
    ...
    def fit(
        ...
        mu_x, sig2_x, theta_x = (
            self.config.mu_x(), self.config.sig2_x(), self.config.theta_x()
        )
        ...
        nb = differential_core.compute_logpmf_values(
            models, obs, exp, float(theta_x[0]), float(theta_x[-1]), theta_x.size
        )
        ...
        for i, sample_index in enumerate(indices):
            ...
            loglik_mu[i], loglik_mu_sig2[i], log_sig2_prior[i] = collapsed, raw, prior

        return Differential(names, mu_x, sig2_x, loglik_mu, loglik_mu_sig2, log_sig2_prior)

    @staticmethod
    def _align(groups, obs, exp, disp_models, selected):
        names = tuple(pd.unique(groups)) if selected is None else tuple(dict.fromkeys(selected))
...
def _fit_invchi2(values):
    ...
    result = scipy.optimize.minimize(
        objective, np.log([5.0, np.median(variance)]), method="L-BFGS-B",
...
def _sig2_prior(sig2_x, weights, nu, sig2):
    log_mass = np.array([invchi2.logpdf(x, nu, sig2) for x in sig2_x]) + np.log(weights)
